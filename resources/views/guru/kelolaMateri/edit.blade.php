                                ...
                                <form action="/kelMateri/{{ $materi->id }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="category" class="form-control @error('category') is-invalid @enderror" id="category">
                                                @foreach($mapels as $mapel)
                                                    <option value="{{ $mapel->id }}" {{ (old('category', $materi->id_mapel) == $mapel->id) ? 'selected' : '' }}>{{ $mapel->mapel }}</option>
                                                @endforeach
                                            </select>
                                            @error('category')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="materi" id="editor" class="form-control @error('materi') is-invalid @enderror">{{ old('materi', $materi->materi) }}</textarea>
                                            @error('materi')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
